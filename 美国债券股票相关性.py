...
# 1. 股票数据 - 获取2015年后的S&P 500指数数据
print("Fetching S&P 500 Index data from Yahoo Finance...")
sp500 = yf.download('^GSPC', start='2015-01-04', end='2024-11-30')
sp500 = sp500[['Close']].reset_index()
sp500.columns = ['Date', 'Stock_Close']
sp500['Date'] = pd.to_datetime(sp500['Date'])

# 2. 加载美国10年期国债收益率数据
print("Fetching US 10-year bond yield data from Yahoo Finance...")
bond_yield_data = yf.download('^TNX', start='2015-01-04', end='2024-11-30')
bond_yield_data = bond_yield_data[['Close']].reset_index()
bond_yield_data.columns = ['Date', 'Bond_Close']
bond_yield_data['Date'] = pd.to_datetime(bond_yield_data['Date'])

# 3. 数据清理和对齐
print("Cleaning and merging data...")
data = pd.merge(sp500, bond_yield_data, on='Date', how='inner')
...
plt.title('US Stock-Bond Rolling Correlation (2015-2024)', fontsize=16)
plt.xlabel('Date', fontsize=12)
plt.ylabel('Correlation', fontsize=12)
plt.legend(fontsize=12)
plt.grid(alpha=0.5)
plt.tight_layout()
plt.savefig('us_stock_bond_rolling_correlation_2015_2024_small_window.png', dpi=300)
plt.show()
...
plt.savefig('us_irf_plot.png', dpi=300)
plt.show()
...
plt.legend()
plt.savefig('us_fevd_plot.png', dpi=300)
plt.show()
...
plt.plot(data['Date'], data['Rolling_Correlation'], label='Rolling Correlation (6-month)', color='blue')
plt.axhline(0, color='red', linestyle='--')
plt.title('Rolling Correlation with Cointegration Insights (US Market)')
plt.xlabel('Date')
plt.ylabel('Correlation')
plt.legend()
plt.grid(alpha=0.5)
plt.tight_layout()
plt.savefig('us_combined_analysis_plot.png', dpi=300)
plt.show()
